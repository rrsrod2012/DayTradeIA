...
function tfToMinutes(tfRaw) {
    ...
    if (s.startsWith("H"))
        return (Number(s.slice(1)) || 1) * 60;
    const n = Number(s);
    return Number.isFinite(n) && n > 0 ? n : 1;
}
function canonicalTF(tfRaw) {
    const m = tfToMinutes(tfRaw);
    return m % 60 === 0 ? `H${m / 60}` : `M${m}`;
}
function bucketStartUTC(d, tfMin) {
...
function detectCrossSignals(candles) {
    ...
    for (let i = 1; i < candles.length; i++) {
        ...
        const reason = side === "BUY" ? `EMA9 cross above EMA21` : `EMA9 cross below EMA21`;
        out.push({
...
/* ---------------- Agregação M1 → TF ---------------- */
function aggregateToTF(m1, tfMin) {
    if (tfMin <= 1) {
...
async function main() {
    ...
    console.log(`[RebuildSignals] symbol=${symbol} timeframe=${tfLabel} (${tfMin}m) from=${from || "-"} to=${to || "-"} dry=${!!dry} limit=${limit ?? "-"}`);
    // 0) Instrument
    ...
    const whereTime = {};
    if (from)
        whereTime.gte = new Date(from.includes("T") ? from : `${from}T00:00:00.000Z`);
    if (to)
    ...
    if (!m1Candles.length) {
        console.error("Nenhum candle M1 encontrado para o período/símbolo informado.");
        process.exit(0);
    }
    // 2) Agregar para o TF alvo
    const agg = aggregateToTF(m1Candles.map((c) => ({
        time: c.time,
        open: c.open,
        high: c.high,
        low: c.low,
    ...
    console.log(`[RebuildSignals] Candles agregados para ${tfLabel}: ${agg.length}`);
    if (!agg.length) {
        console.error("Agregação resultou vazia.");
        process.exit(0);
    }
    if (dry) {
        console.log(`[RebuildSignals] (dry-run) Não haverá gravação de candles/sinais.`);
        process.exit(0);
    }
    ...
    console.log(`[RebuildSignals] Candles ${tfLabel}: created=${createdCandles.length} updated=${updatedCandles.length}`);
    ...
    console.log(`[RebuildSignals] Série ${tfLabel} carregada do banco: ${tfCandles.length}`);
    ...
    console.log(`[RebuildSignals] Sinais detectados (${tfLabel}): ${signals.length}`);
    if (!signals.length) {
        console.log("Nenhum sinal detectado (isso pode acontecer dependendo do período).");
        process.exit(0);
    }
    ...
    console.log(`[RebuildSignals] Concluído. signals created=${created} updated=${updated}`);
